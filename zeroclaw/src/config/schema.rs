...
pub struct ChannelsConfig {
    ...
    /// Channel Talk channel configuration.
    pub channeltalk: Option<ChannelTalkConfig>,
...
/// Channel Talk configuration (webhook receive + REST API send).
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
pub struct ChannelTalkConfig {
    /// Channel Talk API access key (`x-access-key` header).
    pub access_key: String,
    /// Channel Talk API access secret (`x-access-secret` header).
    pub access_secret: String,
    /// Sender names to ignore (e.g. bot's own name to prevent loops).
    #[serde(default)]
    pub ignore_senders: Vec<String>,
}

impl ChannelConfig for ChannelTalkConfig {
    fn name() -> &'static str {
        "Channel Talk"
    }
    fn desc() -> &'static str {
        "Channel Talk platform"
    }
}
