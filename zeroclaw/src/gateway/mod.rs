...
use crate::channels::{
    Channel, ChannelTalkChannel, LinqChannel, NextcloudTalkChannel, SendMessage, WatiChannel,
    WhatsAppChannel,
};
...
pub struct AppState {
    ...
    pub channeltalk: Option<Arc<ChannelTalkChannel>>,
...
pub async fn run_gateway(host: &str, port: u16, config: Config) -> Result<()> {
    ...
    // Channel Talk channel (if configured)
    let channeltalk_channel: Option<Arc<ChannelTalkChannel>> =
        config.channels_config.channeltalk.as_ref().map(|ct| {
            Arc::new(ChannelTalkChannel::new(
                ct.access_key.clone(),
                ct.access_secret.clone(),
                ct.ignore_senders.clone(),
            ))
        });
    ...
    if channeltalk_channel.is_some() {
        println!("  POST /channeltalk    — Channel Talk bot webhook");
    }
    ...
    let state = AppState {
        ...
        linq: linq_channel,
        linq_signing_secret,
        channeltalk: channeltalk_channel,
    ...
    let app = Router::new()
        ...
        .route("/channeltalk", post(handle_channeltalk_webhook))
...
/// POST /channeltalk — incoming message webhook (Channel Talk)
async fn handle_channeltalk_webhook(
    State(state): State<AppState>,
    body: Bytes,
) -> impl IntoResponse {
    let Some(ref channeltalk) = state.channeltalk else {
        return (
            StatusCode::NOT_FOUND,
            Json(serde_json::json!({"error": "Channel Talk not configured"})),
        );
    };
    ...
    for msg in &messages {
        ...
        if state.auto_save {
            let key = format!("channeltalk_{}_{}", msg.sender, msg.id);
            let _ = state
                .mem
                .store(&key, &msg.content, MemoryCategory::Conversation, None)
                .await;
